...
#define RC_LG  A1      ///< Right Circuit Breaker Landing Ger
#define RC_HOOK A3      ///< Right Circuit Breaket Hook
#define RC_FCS4  3    ///< Right Circuit Breaker FCS4
#define RC_FCS3 A2     ///< Right Circuit BreaKer FCS3
...
DcsBios::Switch3Pos wshieldAntiIceSw("WSHIELD_ANTI_ICE_SW", DF_RAIN, DF_ANTIICE);
DcsBios::Switch3Pos canopySw("CANOPY_SW", CN_CLOSE, CN_OPEN);
...
// DcsBios::Potentiometer defogHandle("DEFOG_HANDLE", DF_A );
...
void loop() {
  ...
  // currentValue = analogRead(DF_A);
  // deadZone = (abs(currentValue- joyStickOld) / currentValue)*100;
  // if(deadZone > 90){
  // Joystick.setXAxis(currentValue);  // Set the defog lever position.
  // joyStickOld = currentValue;

  //   // sprintf((char*)data, "JOYX %d", currentValue);
  //   // muxBus.send(0, data, strlen((char*)data)); 
  // }
...
void checkSwitches(){
  if(DcsBios::CheckBus()){
    ...
    if(add == "CANS"){
      if (value == 2) {
      if (wowRight == wowLeft == wowNose == true) {
       outputDebounce(CN_OPEN_MAG, HIGH,100);
        canopyMagHold = true;
      }
      } else {
        outputDebounce(CN_OPEN_MAG, LOW,100);
        canopyMagHold = false;
      }
    }
  }
